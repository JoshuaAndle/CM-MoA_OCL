...
class MoACLIP(nn.Module):

    def __init__(self, model_name, peft_method='adapter', peft_encoder='both', device='cpu'):
        super(MoACLIP, self).__init__()
        self.device = device
    ...
    def forward(self, image, text_tokens=None, is_train=True, val_task_id=None):
        if text_tokens is None:
            text_tokens = self.text_tokens
        logits_per_image, _, image_features, text_features = self.clip_model(image, text_tokens, is_train, val_task_id)
