...
global_subnet_id = None
global_val_subnet_id = None
...
class ResidualAttentionBlock_MoA(ResidualAttentionBlock):
    ...
    def noisy_top_k_gating(self, x, train, w_gate, w_noise, subnet, noise_epsilon=1e-2):
        ...
        """
        # print("Size of x in gating: ", x.shape)
        # print("Size of router in gating: ", w_gate.shape)
        ...
        # print("Shape of topk logits: ", top_k_logits.shape, " ", top_k_indices)
    ...
    def forward(self, x: torch.Tensor, is_train=True):
        ...
        if is_train == False and global_val_subnet_id == -1:
            ...
            # print("\n\n\nnonzero indices: ", nonzero_indices)
    ...
    # #!# No ffn_option == "parallel" check, ensure this is fine
    # def forward(self, x: torch.Tensor):
    #     x = x + self.attention(self.ln_1(x))
...
class Transformer(nn.Module):
    ...
    def __init__(self,
        ...
        if self.res_type == 'moe' and peft_flag:
            self.resblocks = nn.ModuleList([ResidualAttentionBlock_MoA(width, heads, attn_mask, modal, design_details) for _ in range(layers)])
            # self.resblocks = nn.Sequential(*[ResidualAttentionBlock_MoA(width, heads, attn_mask, modal, design_details) for _ in range(layers)])
...
class CLIP(nn.Module):
    ...
    def forward(self, image, text, is_train=True, val_subnet_id=None):
        if image is None:
