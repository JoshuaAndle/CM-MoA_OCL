...
class OnlineSampler(Sampler):
    ...
    def __init__(self,
        ...
        ### Percent of classes disjointly included in each new task (n) and blurrily overlapping between tasks (m)
        ### M selects the number of blurry classes used in each dataset, but the number of overall blurry classes is 1-n
        self.n = n
        self.m = m
        ...
        print("Number of classes: {}, # disjoint: {}, # blurry: {}, for n={}, m={}".format(len(self.classes), self.disjoint_num, self.blurry_num, self.n, self.m))
        ...
        if not self.varing_NM:
            ...
            for i in range(num_tasks):
                logging.info("task %d: disjoint %d, blurry %d" %
                             (i, len(self.disjoint_indices[i]),len(self.blurry_indices[i])))
            ...
            if self.disjoint_num > 0:
                self.disjoint_slice = [0] + torch.randint(0, self.disjoint_num, (num_tasks - 1, ),
            ...
            if self.blurry_num > 0:
                ...
                self.blurry_classes = [class_order[self.disjoint_num + 
                                        self.blurry_slice[i]:self.disjoint_num +
                                        self.blurry_slice[i + 1]].tolist() for i in range(num_tasks)]
            else:
...
class OnlineMultiDatasetSampler(Sampler):
    ...
    def __init__(self,
        ...
        ### This assumes the data_source is of the IndexedDataset wrapper class, accessing the multiDataset object as .dataset
