...
from datasets import get_dataset, get_multipart_dataset
...
class _Trainer():
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.per_task_datasets = kwargs.get("per_task_datasets")
        self.distribution_shift = kwargs.get("distribution_shift_type")
        self.task_id = 0
    ...
    def main_worker(self, gpu) -> None:
        ...
        if self.distributed:
            ...
            if 'SLURM_PROCID' in os.environ.keys():
                ...
                print("| Init Process group {os.environ['RANK']} : {self.local_rank}")
            else:
                self.rank = self.gpu
        ...
        if self.per_task_datasets == False:
            ...
            self.model, self.inp_size = get_model(
                model_name=self.model_name, method=self.method,
            ...
            for task in self.multidatasets_dict.keys():
                _, _, _, n_classes = self.multidatasets_dict[task]
            ...
            for i, task in enumerate(self.multidatasets_dict.keys()):
                self.dataset, self.mean, self.std, _ = self.multidatasets_dict[task]
        ...
        for task_id in range(self.n_tasks):
            ### If using a different dataset for each task, we need to update the train_transforms that will be applied during training 
            if self.per_task_datasets:
                _, self.mean, self.std, _ = self.multidatasets_dict[task_id]
                self.setup_transforms()
            self.task_id = task_id
            if self.method == "joint" and task_id > 0:
                return

            ### Load the next dataset if using different datasets for each task
            # if self.per_task_datasets == True:
                ### Set the offset to the labels to the number of known classes at the start of the task
                # self.label_offset[task_id] = len(self.exposed_classes)
                
                # if task_id > 0: 
                    # self.dataset, self.mean, self.std, self.n_classes = get_multipart_dataset(self.dataset_name, task_id)
                    # self.setup_dataset()
            #         train_dataset = IndexedDataset(self.train_dataset)
            #         self.train_dataloader = DataLoader(train_dataset,
            #                                             batch_size=self.batchsize,
            ...
            #                                             num_workers=self.n_worker,
            #                                             pin_memory=True)
            # elif self.per_task_datasets == False:
            ...
            for i, (images, labels, idx) in enumerate(self.train_dataloader):
                ...
                # if self.per_task_datasets == True:
                #     labels += self.label_offset[task_id]

                loss, acc = self.online_step(images, labels, idx)
                running_train_loss += loss
                running_train_acc += acc

                self.report_training(samples_cnt, running_train_loss/(i+1), running_train_acc/(i+1))
            ...
            ### Added no_grad() to match other evaluate calls
            with torch.no_grad():
                # if self.per_task_datasets == False:
                test_sampler = OnlineTestSampler(self.test_dataset, self.exposed_classes)
                # else:
                #     test_sampler = OnlineMultiDatasetTestSampler(self.test_dataset, self.exposed_classes)

                self.test_dataloader = DataLoader(self.test_dataset,
                                             batch_size=self.batchsize * 2,
                                             sampler=test_sampler,
                                             num_workers=self.n_worker)
                eval_dict = self.online_evaluate(self.test_dataloader)

            #! after training done
            self.report_test(num_eval, eval_dict["avg_loss"], eval_dict['avg_acc'])

            if self.distributed:
                confusion_matrix = torch.tensor(eval_dict['confusion_matrix'], device=self.device)
            ...
            if self.is_main_process() and 'confusion_matrix' in eval_dict:
                np.save(os.path.join(self.log_dir, f'seed_{self.rnd_seed}_T{task_id}_confusion_matrix.npy'), eval_dict['confusion_matrix'])
        ...
        if self.is_main_process():
            ...
            logging.info(f"A_auc {A_auc:.5f} | A_avg {A_avg:.5f} | A_last {A_last:.5f} | F_last {F_last:.5f}")
    ...
    def add_new_class(self, labels):
        ...
        self.exposed_classes_names = [self.train_dataset.classes_names[i] for i in self.exposed_classes]
    ...
    #*# To-Do: This may not work with multipart datasets. Check once implementing parallel computing
